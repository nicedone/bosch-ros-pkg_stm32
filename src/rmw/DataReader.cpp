...
DataReader::DataReader(const char* topic, const char* callerID, const char* md5sum, const char* msgType)
{
    ...
    qHandle = xQueueCreate(1, RX_QUEUE_MSG_SIZE);
    connectionID = 0;
    // TODO: make a unique task name
    xTaskCreate(task, (const signed char*)"topic", RX_QUEUE_MSG_SIZE + 112, (void*)this, tskIDLE_PRIORITY + 2, NULL);
}
...
void DataReader::enqueueMessage(const char* msg)
{
    ...
    if (qHandle)
    {
        if (xQueueSend(qHandle, &data, 100))
        {
