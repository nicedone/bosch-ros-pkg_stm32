...
void RMW::tcptask(void* p)
{
    ...
    self->TCPqHandle = xQueueCreate(4, sizeof(TCPData));
    for(;;)
    {
        if (xQueueReceive(self->TCPqHandle, &tcpData, 100))
        {
            os_printf("Q recv\n");
        port++;
                ...
                if (pos4 > pos3)
                {
                    ...
                    if (dw != NULL)
                    {
                        ...
                        if (connectionID > -1)
                        {
                            ...
                            XMLRequest* response = new TopicResponse(SENDER_IP_ADDR, UDP_LOCAL_PORT, connectionID);
...
DataReader* RMW::addDataReader(const char* topic, const char* callerID, const char* md5sum, const char* msgType)
{
    ...
    sendRequest(req->getData(), 11311, connectPublishers, dr);

    return dr;
}

DataWriter* RMW::addDataWriter(const char* topic, const char* callerID, const char* msgType)
{
    static uint16_t lastIndex = 0;
    ...
    sendRequest(req->getData(), 11311, connectSubscribers, dw);
        ...
        while(1)
        {
            ...
            if (pos3 > pos2)
            {
                ...
                if (strcmp(ip, "10.3.84.99")) // TODO: replace this with a method to check if ip is not equal self ip
                {
                    ...
                    requestTopic(self, SERVER_IP_ADDRESS, port);
                }
            }
            pos = pos3;
        }
...
{
    XMLRequest* req = new TopicRequest("requestTopic", MASTER_URI, dr->getCallerID(), dr->getTopic(), dr->getMd5Sum(), dr->getMsgType());
